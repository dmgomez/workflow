...
class RolController extends Controller
{
	...
	public function accessRules()
	{
		return array(
			...
			array('allow', // allow authenticated user to perform 'create' and 'update' actions
				'actions'=>array('create','update', 'admin', 'delete', 'ActivarActividades', 'DeleteGrid'),
				'users'=>array('@'),
			),
	...
	*/
	...
	{
		$actividadesAsociadas = VisActRol::model()->findAllByAttributes(array('id_rol'=>$id));
		$empleadosAsociados = VisEmpleado::model()->findAll(array("select"=>"id_rol, id_cargo, nombre_cargo, nombre_persona", "condition"=>"id_rol like '%".$id."%'", "order"=>"nombre_cargo, nombre_persona"));

		$this->render('view',array(
			'model'=>$this->loadModel($id),
			'actividadesAsociadas'=>$actividadesAsociadas,
	...
	{
		$actividades = Actividad::model();
		$rol = Rol::model();
		$actividadesProceso = $actividades->findAll(array('condition'=>'id_proceso = '.(int) $_POST['idProceso']));
		
		$result = array(
			'actividad' => CHtml::activeCheckBoxList($rol, '_actividad', 
	...
	public function actionDelete($id)
	{
		...
		catch (Exception $e) 
		{
			if(strpos($e, 'Foreign key violation'))
		  	{
		  		Yii::app()->user->setFlash("error", "No se puede eliminar el registro. El rol está siendo utilizado.");
		  		$this->redirect(array('admin'));
		  	}
		}
	...
	public function actionDeleteGrid($id)
	{
		...
		if(Yii::app()->request->isPostRequest)
		{
			...
			catch (Exception $e) 
			{
				if(strpos($e, 'Foreign key violation'))
			  	{
			  		$result = array('success' => false, 'message' => 'No se puede eliminar el registro. El rol está siendo utilizado.');
			  	}
			}
		}
		else
